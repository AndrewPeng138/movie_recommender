...
/**
 * Minimum similarity, as a fraction of the candidate's *best* match, for a pick to be named.
 *
 * Filtering on `similarity > 0` was effectively no filter at all. Measured on a real result:
 * Black Panther: Wakanda Forever scored 0.5371 against Black Panther, 0.0169 against Up, and 0.0101
 * against The Terminator — so the card claimed "Because you liked: Black Panther, Up, The Terminator"
 * when the last two contributed 3% and 2%.
 *
 * Relative rather than absolute because cosine values vary widely per film: one candidate's best
 * match may be 0.54 and another's 0.15, so a fixed cutoff would over-filter the weaker one. At 0.25
 * the example above correctly keeps only Black Panther.
 */
const SIMILAR_TO_MIN_RATIO = 0.25;

/**
 * Feature keys computed but never shown as a reason.
 *
 * Both still contribute to similarity — they are suppressed only in the explanation.
 *
 * - `dec:*` (decade) says nothing the release year already on the card doesn't.
 * - `lang:en` earns an IDF of 1.16, the lowest of any feature in the corpus, because nearly every
 *   film is English. Telling someone two films match because both are in English is noise. A
 *   *non*-English match is genuinely informative, so only English is hidden.
 */
const HIDDEN_EXPLAIN_KEYS = /^(dec:|lang:en$)/;
    ...
    return {
        ...
        finalise(scored, limit) {
            ...
            return selected.map(entry => {
                ...
                // Ask for extra contributions, then drop the ones not worth showing, so a card still
                // ends up with explainLimit useful reasons rather than three plus a filler.
                // See HIDDEN_EXPLAIN_KEYS for what is dropped and why.
                const contributions = (best
                    ? explain(entry.vector, best.entry.vector,
                        new Map([...best.entry.labels, ...entry.labels]), p.explainLimit + 3)
                    : []
                ).filter(c => !HIDDEN_EXPLAIN_KEYS.test(c.key)).slice(0, p.explainLimit);
                ...
                return {
                    ...
                    // Only the picks this film genuinely resembles — at least SIMILAR_TO_MIN_RATIO of
                    // its best match, capped at SIMILAR_TO_LIMIT. topPicks is already sorted by
                    // similarity, so [0] is the strongest.
                    similarTo: entry.topPicks
                        .filter(s => entry.topPicks[0]
                            && s.similarity >= entry.topPicks[0].similarity * SIMILAR_TO_MIN_RATIO)
                        .slice(0, SIMILAR_TO_LIMIT)
                        .map(s => s.pick.title),
