...
const DAY = 24 * HOUR;
/** One year in milliseconds — the TTL used for a frozen evaluation corpus. */
const YEAR = 365 * DAY;

/**
 * Optional global TTL override, in milliseconds, from `CACHE_TTL_MS`.
 *
 * Exists for the evaluation corpus. Ordinary TTLs are about freshness, but a corpus needs the
 * opposite guarantee: **reproducibility**. A baseline measured against a corpus that silently expired
 * and re-fetched is not comparable to a later run, because TMDB's data may have moved underneath it.
 * Corpus builds therefore set this to a year, freezing the data until refreshed deliberately.
 *
 * @type {number|null}
 */
const TTL_OVERRIDE = process.env.CACHE_TTL_MS ? Number(process.env.CACHE_TTL_MS) : null;
...
class DiskStore {
    ...
    /**
     * Rewrites every entry's expiry to `ttlMs` from now, preserving its value.
     *
     * Lets an existing corpus be frozen in place rather than re-fetched — important when the corpus
     * cost ~19,000 TMDB requests to build.
     *
     * @param {number} ttlMs - New lifetime, measured from now.
    ...
    reExpire(ttlMs) {
        ...
        for (const shard of fs.readdirSync(this.dir)) {
            ...
            for (const name of fs.readdirSync(shardPath)) {
                ...
                const file = path.join(shardPath, name);
                try {
                    const entry = JSON.parse(fs.readFileSync(file, 'utf8'));
                    ...
                    // Corrupt entry — leave it; get() already treats it as a miss.
                }
            }
        }
...
class Cache {
    ...
    async wrap(key, ttlMs, produce) {
        ...
        this.store.set(key, value, TTL_OVERRIDE ?? ttlMs);
        return value;
    }

    /**
     * Extends every stored entry's expiry to `ttlMs` from now.
     *
     * Used by scripts/freeze-corpus.js to convert an already-built corpus into a frozen one without
     * re-issuing thousands of TMDB requests.
     *
     * @param {number} ttlMs - New lifetime, measured from now.
     * @returns {number} Number of entries updated.
     */
    reExpire(ttlMs) {
        return typeof this.store.reExpire === 'function' ? this.store.reExpire(ttlMs) : 0;
    }
...
module.exports = { createCache, Cache, MemoryStore, DiskStore, HOUR, DAY, YEAR };
