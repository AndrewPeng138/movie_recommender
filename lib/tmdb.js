...
const { rankSearchResults } = require('./search-rank');
 ...
 *
 * Results are re-ordered before returning — TMDB sorts by a *trending* metric, which buries
 * recognisable films behind obscure ones that happen to be moving. See lib/search-rank.js.
 *
 * Ranking happens *after* the cache read, deliberately: the cache holds TMDB's raw payload, so
 * changing the ranking rules takes effect immediately rather than waiting an hour for entries to
 * expire. The spread also avoids mutating the cached object, which is shared with every later read.
 *
...
async function searchMovies(query) {
    ...
    const payload = await tmdb('/search/movie', { query }, { ttlMs: HOUR });
    return { ...payload, results: rankSearchResults(payload.results, query) };
}
