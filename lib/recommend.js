...
async function recommend(pickIds, { generator = 'tmdb', limit = RESULT_LIMIT } = {}) {
    ...
    const ranked = rank.rankCandidates(groups, pickIdSet).slice(0, limit);

    // 5. Shape for the client. Only fields the UI renders are sent — the full TMDB payload is large
    //    and most of it is unused.
